...
public class Node {
    ...
    Node () {}
    // Конструктор
    Node(int d, int leng, String files) {
        all_remember = leng;
        this.index = d;
    ...
    public static Node initializing (Node list, String files, int leng) throws IOException {
        for (int i = 0; i < 32; i++) {
            // Создаем новый узел с заданными данными
            Node new_node = new Node(i, leng, files);
            new_node.next = null;
        ...
        File ff = new File(list.head.file);
    ...
    static public int return_kash (Node list) throws IOException {
        Path path = Path.of("D:\\OS_Laba_5\\recorses\\kash");
    ...
    static public void help(Node list) throws IOException {
        FileWriter fileWriter = new FileWriter("D:\\OS_Laba_5\\recorses\\kash");
        Node last = list.head;
        int i = 0;
        while (last.next != null) {
            fileWriter.write(i + ": [" + last.messege + "]\n");
    ...
    public static boolean have_or_not (Node list) {
        int count_false = 0;
        Node last = list.head;
        while (last.next != null) {
            if (last.messege.length() < 6)
                count_false++;
            last = last.next;
        }
